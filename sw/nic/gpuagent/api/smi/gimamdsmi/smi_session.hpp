...
private:
    ...
    /// Must be recursive: smi_state::init() holds the lock via an outer
    /// smi_session while calling smi_discover_gpus(), which itself opens
    /// a nested smi_session via AGA_SMI_SESSION_GUARD() — same thread,
    /// same mutex, so a plain std::mutex deadlocks on the nested lock().
    static std::recursive_mutex& init_mutex_(void);
